...
// ── PdfProxy — carga PDFs de SharePoint vía proxy autenticado ──────────────────

const PdfProxy = ({ archivoUrl }) => {
  ...
  useEffect(() => {
    ...
    descargarArchivoIntranet('reglamento', filename)
      .then(async blob => {
        // Forzar application/pdf para que el iframe lo renderice en lugar de descargarlo
  ...
  if (error) return (
    <div className="w-full h-16 flex items-center justify-center text-xs text-red-500 bg-red-50 rounded-lg border border-red-100">
      No se pudo cargar la previsualización
    </div>
  );
  if (!blobUrl) return (
    <div className="w-full h-16 flex items-center justify-center text-xs text-slate-400 bg-slate-50 rounded-lg animate-pulse">
      Cargando previsualización…
    </div>
  );
  return (
    <iframe src={blobUrl} className="w-full border border-slate-200 rounded-lg" style={{ height: '400px' }} title="PDF Viewer" />
  );
};
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.archivo_url && (
                        <div className="mb-5">
                          ...
                          <PdfProxy archivoUrl={item.archivo_url} />
                        </div>
                      )}
                      {item.contenido && (
                        <div className="space-y-2">
