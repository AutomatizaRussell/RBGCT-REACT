...
const UserDashboard = () => {
  ...
  const isSQF =
    location.pathname.includes('/app/sqf') &&
    Boolean(empleadoData?.acceso_formularios_sqf)
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        // Los stats del dashboard no son críticos.
        // Si fallan, no se interrumpe la experiencia del usuario.
      } finally {
          ...
          {isHome ? (
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2 lg:p-8">
                  <div className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 pb-4">
                    <div>
                      <h3 className="text-base font-semibold text-[#001871]">
                      ...
                      <div key={task.label} className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="mb-2 flex items-center justify-between gap-2">
                            ...
                            <span className={`text-sm font-semibold tabular-nums ${task.textColor}`}>
                              {task.count}
                            </span>
                          </div>
                          ...
                          <div className="h-1.5 overflow-hidden rounded-sm bg-slate-100">
                            <div
                              className={`h-full rounded-sm ${task.color} transition-all duration-700`}
                              style={{
                                width:
                                  taskStats.total > 0
                                    ? `${(task.count / taskStats.total) * 100}%`
                                    : '0%',
                              }}
                            />
                          </div>
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm lg:p-8">
                  <h3 className="border-b border-slate-200 pb-4 text-sm font-semibold text-[#001871]">
                    Accesos rápidos
                  </h3>
            ...
            <div className="mx-auto max-w-6xl animate-in fade-in duration-500">
              <div className="mb-8 border-b border-slate-200 pb-6">
                ...
                <h3 className="mt-2 text-lg font-semibold text-[#001871]">
                  Herramientas
                </h3>
...
const NotifItem = ({ tarea, vencida, onNavigate, fmtFecha }) => (
  ...
  >
    ...
    <div className="min-w-0 flex-1">
      <p className="truncate text-xs font-bold leading-snug text-[#001871]">
        {tarea.titulo}
      </p>
        ...
        {tarea.fecha_vencimiento && (
          <span
            className={`flex items-center gap-1 text-[10px] font-medium ${
              vencida ? 'text-slate-700' : 'text-slate-500'
            }`}
          >
...
const KpiCard = ({
  ...
  >
    <div className="mb-4 flex items-start justify-between">
      <div
        className={`flex h-10 w-10 items-center justify-center rounded-md border border-slate-100 ${iconBg}`}
      >
    ...
    <p className="text-2xl font-black tabular-nums leading-none text-[#001871] sm:text-3xl">
      {value}
    </p>

    <p className="mt-3 truncate text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">
      {label}
    </p>
